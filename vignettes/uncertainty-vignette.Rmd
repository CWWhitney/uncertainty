---
title: "Uncertainty"
author: Cory Whitney^a^, Eike Luedeling^a^ <br /> <small>^a^INRES-Horticultural Sciences, University of Bonn, Auf dem Huegel 6, 53121 Bonn, Germany <small/>
output:
...
`varviolin()` shows violin plots of influencing variable {in_var} values (x) and outcome variable {out_var} (y) values, with six different intervals of influencing variable {in_var} values. Plot made with `ggplot2()` (Wickham 2016).
...
```{r, fig.width=7, fig.height=7}
uncertainty::varkernelslicerange(in_var, out_var, 100, 200)
```
