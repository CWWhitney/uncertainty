...
To tell the difference in two distributions determined by the `varkernelslice()` ranges we can test the difference. We can extract the data with the `varslice_resample()` function.

```{r}

A <- varslice_resample(in_var, out_var, expectedin_var = 40, n_runs = 10)
B <- varslice_resample(in_var, out_var, expectedin_var = 10, n_runs = 10)
